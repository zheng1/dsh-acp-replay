...
export function apply(ctx: Context, config: AcpConfig): void {
  ...
  const implementation = {
    async initialize(_params: InitializeRequest): Promise<InitializeResponse> {
      ...
      return {
        ...
        agentCapabilities: {
          ...
          // Replaying a persisted log is what makes a resumed session visible
          // to the client that created it; `session/resume` alone does not.
          loadSession: true,
        },
        authMethods: [],
      }
    ...
    /**
     * Restore a persisted session and replay its transcript.
     *
     * ACP expects the agent to stream the whole conversation back as
     * `session/update` notifications before this call returns, so a client that
     * treats the agent as the transcript authority can render history after its
     * own restart.
     */
    async loadSession(params: LoadSessionRequest, signal: AbortSignal): Promise<LoadSessionResponse> {
      ...
      return (async (): Promise<LoadSessionResponse> => {
        // 'read' observes without taking the single write owner, so the log can
        // be snapshotted before the restored Agent starts appending to it.
        const handle = await persistence.open(sessionId, 'read', { signal })
        let persisted
        let transcript
        try {
          persisted = handle.header
          transcript = (await handle.read(0, undefined, { signal })).events
        } finally {
          await handle.close()
        }
        if (persisted.origin === 'subagent' || persisted.parentSession !== undefined) {
        ...
        try {
          const configOptions = await record.configOptions(signal)
          for (const update of await replaySessionUpdates(ctx, record.agent.session, transcript)) {
            await notify({ sessionId, update })
          }
          return { configOptions }
        } catch (error: unknown) {
          sessions.delete(sessionId)
          await record.close('session/load replay failed')
          throw error
        }
      })().finally(() => { activating.delete(sessionId) })
    },
